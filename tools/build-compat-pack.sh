...
PY
  ...
  if [[ -n "${STS2_ORIGINAL_V1100_ROOT:-}" ]]; then
    STS2_ORIGINAL_V1100_ROOT="$(_sts2_abs "$STS2_ORIGINAL_V1100_ROOT")"
    STS2_ORIGINAL_V1100_REFERENCE_DIR="${STS2_ORIGINAL_V1100_REFERENCE_DIR:-$STS2_ORIGINAL_V1100_ROOT/.godot/mono/temp/bin/Debug}"
  fi
  if [[ -n "${STS2_ORIGINAL_V1110_ROOT:-}" ]]; then
    STS2_ORIGINAL_V1110_ROOT="$(_sts2_abs "$STS2_ORIGINAL_V1110_ROOT")"
    STS2_ORIGINAL_V1110_REFERENCE_DIR="${STS2_ORIGINAL_V1110_REFERENCE_DIR:-$STS2_ORIGINAL_V1110_ROOT/.godot/mono/temp/bin/Debug}"
  fi
  ...
  if [[ -n "${STS2_ORIGINAL_V1100_REFERENCE_DIR:-}" ]]; then STS2_ORIGINAL_V1100_REFERENCE_DIR="$(_sts2_abs "$STS2_ORIGINAL_V1100_REFERENCE_DIR")"; fi
  if [[ -n "${STS2_ORIGINAL_V1110_REFERENCE_DIR:-}" ]]; then STS2_ORIGINAL_V1110_REFERENCE_DIR="$(_sts2_abs "$STS2_ORIGINAL_V1110_REFERENCE_DIR")"; fi
fi
...
if [[ -z "${COMPAT_REFERENCE_DIR:-}" ]]; then
  case "$REFERENCE_FLAVOR" in
    ...
    original-v0.109.0)
      ...
      ;;
    original-v0.110.0)
      COMPAT_REFERENCE_DIR="${STS2_ORIGINAL_V1100_REFERENCE_DIR:-$ROOT/refs/original-v0.110.0}"
      ;;
    original-v0.111.0)
      COMPAT_REFERENCE_DIR="${STS2_ORIGINAL_V1110_REFERENCE_DIR:-$ROOT/refs/original-v0.111.0}"
      ;;
    runtime|*)
